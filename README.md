...
2. **Vivliostyleツール群と同居しないもの**:
	...
	日本語小説にフォーカスした変換のみを行うマークアップとhtml変換を実現するものが欲しくなり、開発しています。
...
| **強制合字** | `[l[AB]r]` | `<span class="ltlbg_forcedGouji1/2">` | 2文字を1文字分の幅に詰め込む。 CSSで簡易的に合字を作る|
...
※その他、`！`や`？`の後のスペース自動挿入や、以下の記号の自動統一が行われます。また、行末や閉じ括弧の前など、文脈的に不要と判断される半角・全角スペースは自動的に除去されます。
  - `♡` `♥` → `❤`
  - `☆` → `★`
  - `□` → `■`
  - `♫` `♬` → `♪`
  - 連続する`―`は1つにまとめられます(スタイルで倍幅にする前提)。
  - `！！`、`！？`、`？！`、`？？` は、1文字幅に収まる半角のペア（`!!`, `!?`, `?!`, `??`）に変換されます。
...
ただし、この変換は完全な可逆処理ではありません。例えば、変換の過程で正規化されたスペースの扱いなど、一部の表現は元のテキストと完全には一致しない可能性があります。
