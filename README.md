...
| branch           | ver       | status                                       
| ---------------- | --------- | -------------------------------------------- 
| main             | 0.81      | txt→html、html→txt、一通り実装。パイプライン化。
...
**等幅フォントを使用し、文字が方眼に並ぶことを前提にします。**

## ブランチの説明
- develop-root…開発中のメインブランチ。機能ごとに中間ファイルを使う形で文字列置換処理が分離している。コメントも機能ごとに付与されている。
- main…develop-rootで一段落ついてある程度まとまったもの。develop-rootでは分離していた処理を、機能ごとに分離せずにパイプラインで結合する(ワンライナーに近くなる)。bashスクリプトの都合で、パイプライン中はコメントを挿入できないのでコメントもない。
- 他…機能ごとに開発してたり、バグの修正してたり。
...
| 半角英数記号     | ！や？、英数の重なり | `<span class="ltlbg_tcyA">`                  | 2字のみ。強制半角縦中横化
...
| 縦中横           | `^XX^`               | `<span class="ltlbg_tcyM">`                              | 半角1〜2字のみ
...
### 括弧類ブロックにおける便宜的な段落について
例えば以下の文章で
```
　美女、の一言で表現するのは烏滸がましく、彼女一人がこの鬱陶しい光彩に負けていない。それどころか夕闇
を切り裂くこの光すべてが彼女を照らし出すための舞台装置にさえ見えた。
色鮮やかな光に演出されたように、女は男の方を振り返った。綾を付けているわけでもないのに蠱惑的で、同時
に洗練された動きにはキレがあり、ただのパート主婦には似つかわしくない。
「ん？　なに？　ナンパ？」
「違う違う（笑）　もし一人同士だったら、これから遊ばない？って話」
「ナンパじゃない。こんなオバサン捕まえて、なにいってるの？　キミまだ若いじゃない、大学生？」
「俺年上とかチョー守備範囲だし、むしろストライク？&emsp;でもオバサンって言うには、早いんじゃない？」
「ま、そういうの悪い気はしないけどね、生憎と一人、人と待ち合わせしているの」
「えー、カレシ？」
　彼女がここに立てばこうして男が五人でも十人でも釣れる；この場にいる若々しい少女、キラキラした娘達を
差し置いて、地味な姿のパート主婦の周りに男が群がるのだ。だが、彼女はナンパ待ちでここに立っているわけ
ではない。その言葉通り、人と待ち合わせをしているのだ。
```

段落を`行頭1字下げ`で表現する場合はくっついても問題ないが、このアプリでは段落を空行で表現しようとしている。

連続する`「」`、`（）`、`〝〟`のブロックを一つの便宜的に段落として扱うことで以下のように表示したい。

```
　美女、の一言で表現するのは烏滸がましく、彼女一人がこの鬱陶しい光彩に負けていない。それどころか夕闇
　を切り裂くこの光すべてが彼女を照らし出すための舞台装置にさえ見えた。
　色鮮やかな光に演出されたように、女は男の方を振り返った。綾を付けているわけでもないのに蠱惑的で、同
　時に洗練された動きにはキレがあり、ただのパート主婦には似つかわしくない。
<<< 段落ではないが、ここに空白が開く必要がある
「ん？　なに？　ナンパ？」
「違う違う（笑）　もし一人同士だったら、これから遊ばない？って話」
「ナンパじゃない。こんなオバサン捕まえて、なにいってるの？　キミまだ若いじゃない、大学生？」
「俺年上とかチョー守備範囲だし、むしろストライク？&emsp;でもオバサンって言うには、早いんじゃない？」
「ま、そういうの悪い気はしないけどね、生憎と一人、人と待ち合わせしているの」
「えー、カレシ？」
<<< ここは段落なので、それに従って空白があく
　彼女がここに立てばこうして男が五人でも十人でも釣れる；この場にいる若々しい少女、キラキラした娘達を
　差し置いて、地味な姿のパート主婦の周りに男が群がるのだ。だが、彼女はナンパ待ちでここに立っているわ
　けではない。その言葉通り、人と待ち合わせをしているのだ。
```

これを実現するための便宜的な段落として特別な判定処理を実施する。
...
- txtの章
  - 行頭`[chapter(:章Index)]`の文字から、次の行頭`[chapter(:章Index)]`の文字の手前まで(Pixiv小説)
    ...
    - この`章Index`は`章ラベル`ではない(同じにしても良い)。
- htmlの章
  - `章クラス`を指定した`section`タグに囲まれた範囲

章区切りは、開始時前方に3行分の空白を持つ。
...
章タイトルは、それ自体2行分の幅をもち、直前と直後に1行分の空白を持つ。

但し、直前の行が章区切りの場合は、直前の1行空白は持たない。
...
- txt
  ...
  - `｜母字《ルビ》`による型式(なんかいろいろで使うやつ)
- html
  - `ルビクラス`を指定した`<ruby>`タグと`ルビ用`カスタムデータ属性と`::before`疑似要素を使用した行間干渉の少ないルビ型式
    - 普通の`ruby`タグ+`rt`タグでは行間が開きすぎるため
    - 普通の`ruby`タグ+`rt`タグでは、ルビ文字が母字より長い場合に、1文字分未満の空白が追加されてズレるため

> html→txtでは、`{母字｜ルビ}`に変換する

### 振り方
- 母字数=ルビの字数の場合、モノルビ風とする(改善中)
- 母字数<ルビの字数の場合、始点を合わせて、ルビを後ろにはみ出す
  - 大きく値が違っても母字側に空間は増やさない。
- 母字数>ルビの字数の場合、可能な限り中寄せ、字間を1文字開く。
  - 大きく値が違っても字間は変化させない。細かくは手動で整備してもらう。
...
- html
  - `圏点クラス`を指定した`ruby`タグ。
  - `rt`タグと、`圏点用`カスタムデータ属性に、基底文字と同じ文字数の圏点を設定する。
  - デフォルトは黒ゴマ

> `emphasis`ではなく`ruby`にするのは、ルビと同様に行間干渉を減らす目的。
...
### これはなに？
(「忄実」)[https://entamevr.com/quiz-kanji-yomikata]みたいな創作漢字を表現するための戯れ。

偏と旁のものにしか対応していないが、改造すれば冠と足、囲と中みたいなものでも作れるでしょう。

[t[艹艸]b]とか[o[口回]i]作って遊ぶためのもの。

（自動変換に対応しているのは`[l[偏旁]r]`だけです）
...
- txt
  - 行頭から始まる「」、（）、〝〟記述されたまま
- html
  - カッコ類は、「」、（）、〝〟それぞれの`括弧クラス`を指定した`span`タグの範囲
  - 範囲的には`div`だが、`div`はブロック要素のため、勝手に改行が入り可読性が落ちるので`span`
...
区切り文字の種類としてコロンとセミコロンを使うため、縦書き時に崩れないよう補正します。

- コロン:縦書き時にも回転するが、フォントによっては回転しない。等幅に従わない。
- セミコロン:縦書き時に回転しない。等幅に従わないこともある。

これを修正するために、独自クラスを用意して自動的に変換する。

### コロン
- txt
  - 書かれたままの`：`あるいは半角の`:`
- html
  - `コロン用クラス`を指定した`span`タグ
  - 半角の場合に全角に置換する。
...
- txt
  - 書かれたままの`；`あるいは半角の`;`
- html
  - `セミコロン用クラス`を指定した`span`タグ
  - 半角の場合に全角に置換する。
  - セミコロンは縦書きにしても回転しないので、強制的に回転する。
    - 縦中横とは逆方向に回転させる必要がある
