...
> 行頭に登場するクラス指定なしのbrタグは、`1行空けクラス`を**強制適用**する
...
またhtmlからtxtへ置換する際には、`段落クラス`を指定した`p`タグを判断して、`行頭空白文字`へ置換する。
...
- txtの章
  ...
  - `§`か`◆`の指定の文字で始まる1行から、次の行頭`§`か`◆`の手前まで
> 3行以上連続する空行は、予め`§`の文字へ置換する

> htmlからtxtへ置換する際は`§`文字とする(`◆`へは戻らない)
...
> `§`にて章を記すとき、"§"の文字を章タイトルとしたい場合には、`章クラス`の`before`や`after`の疑似要素にて指定すること。
> 
